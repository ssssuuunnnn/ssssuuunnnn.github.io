...
雖然目前所做的範例使用到 Fleet Mode 也算是殺雞用牛刀，在實際開發中，並非所有任務都需要啟動 Fleet 模式派出一整支 subagents 艦隊。盲目拆分反而會增加主代理（Orchestrator）在合併程式碼與溝通上的成本。另外使用 Fleet Mode 某種程度我認為可以精簡 token 的使用，因為不同的 subagent 可以指定使用不同的 model 使用，每種 model 的 token 計價方式也稍有不同。
